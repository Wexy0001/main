...
//RANK KISMI//
client.ranks = [
  { role: "899273632650510351", coin: 2000 },//başlangıç yt nin bi üstünden başlayarak gir 
  { role: "899273632650510352", coin: 3000 },//unutma!
...
fs.readdir('./src/commands/', (err, files) => {
  ...
  console.log(`[Acer] ${files.length} komut yüklenecek.`);
  files.forEach(f => {
    fs.readdir("./src/commands/" + f, (err2, files2) => {
      files2.forEach(file => {
        let props = require(`./src/commands/${f}/` + file);
        console.log(`[Acer KOMUT] ${props.conf.name} komutu yüklendi!`);
