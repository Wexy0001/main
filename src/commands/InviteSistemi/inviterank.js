...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let tagged;
    if (conf.tag && conf.tag.length > 0) tagged = invMember ? message.guild.members.cache.filter((m) => invMember.some((x) => x.userID === m.user.id) && m.user.discriminator.includes(conf.tag)).size : 0;
    else tagged = 0;
