...
let member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.author);
    ...
    return }
       ...
       let ekipRolu = conf.ekipRolu
 
    let taglilar = message.guild.members.cache.filter(s => s.user.discriminator.includes(ayar.tag) && !s.roles.cache.has(ekipRolu))
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`)
    ...
    message.guild.members.cache.filter(s => s.user.discriminator.includes(ayar.tag) && !s.roles.cache.has(ekipRolu)).map(x=> x.roles.add(ayar.ekipRolu))                
    }
