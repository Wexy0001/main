...
module.exports = {
    ...
    run: async (client, message, args, embed) => {
      let tag = conf.tag
      let tag2 = conf.tag
      let etiket = conf.tag3
      let rol = conf.ekipRolu
      let taglilar = message.guild.members.cache.filter(s => s.user.discriminator.includes(tag) && !s.roles.cache.has(rol))
      let taglilar2 = message.guild.members.cache.filter(s => s.user.discriminator.includes(tag2) && !s.roles.cache.has(rol))
