...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  async function TagAlıncaKontrol() { // Tag alınca tarama
    ...
    const members = guild.members.cache.filter(member => member.user.discriminator.includes(conf.tag) && !member.roles.cache.has(conf.jailRole) && !member.roles.cache.has(conf.ekipRolu)).array().splice(0, 10)
  ...
  async function TagBırakanKontrol() { // Tagı olmayanın family rol çekme
    const guild = this.client.guilds.cache.get(settings.guildID)
    const members = guild.members.cache.filter(member => !member.user.discriminator.includes(conf.tag) && !member.user.bot && member.roles.cache.has(conf.ekipRolu)).array().splice(0, 10)
    for await (const member of members) {
