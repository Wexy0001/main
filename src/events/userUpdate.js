...
module.exports = async (oldUser, newUser) => {
    ...
    if (oldUser.discriminator.includes(conf.tag) && !newUser.discriminator.includes(conf.tag)) {
...
Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.discriminator.includes(conf.tag)).size}
         `);
      channel.wsend(embed);
      } else if (!oldUser.discriminator.includes(conf.tag) && newUser.discriminator.includes(conf.tag)){
...
Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.discriminator.includes(conf.tag)).size}
    `);
      channel.wsend(embed);
      kanal.wsend(new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} üyesi ${conf.tag} tagımızı alarak ailemize katıldı! Ailemiz ${guild.members.cache.filter(x => x.user.discriminator.includes(conf.tag)).size} kişi oldu!`)).then(x=>x.delete({timeout: 5000}))
...
await bannedTag.findOne({ guildID: guild.id }, async ( err, res) => {
 ...
 if (!oldUser.discriminator.includes(x) && newUser.discriminator.includes(x)) {
